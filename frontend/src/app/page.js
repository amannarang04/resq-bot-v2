...
const CRISIS_CARDS = [
  ...
  { icon: '🏥', label: 'Medical', prompt: 'Medical emergency hai turant help chahiye' },
...
export default function Home() {
  ...
  const sendMessage = async (customMsg) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: '❌ Server se connection nahi ho pa raha.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSOS = async () => {
    ...
    playAlert();
    const locationText = await getLocation();
    await sendMessage(`🆘 SOS EMERGENCY! Mujhe turant madad chahiye!${locationText ? ` Mera location: ${locationText}` : ''}`);
    setTimeout(() => setSosActive(false), 3000);
  };

  const clearHistory = () => {
    setMessages([{ role: 'bot', text: '🚨 Chat history clear ho gayi. Main ResQ Bot hoon, kaise madad kar sakta hoon?' }]);
  ...
  return (
    <main style={{
      ...
      {showSidebar && (
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: '24px', padding: '32px', width: '320px',
            border: '1px solid rgba(255,255,255,0.1)',
          }} onClick={e => e.stopPropagation()}>
            <h2 style={{ color: '#fff', fontSize: '20px', fontWeight: 800, marginBottom: '24px', textAlign: 'center' }}>
              📞 Emergency Numbers
            </h2>
            {EMERGENCY_NUMBERS.map((n, i) => (
              <a key={i} href={`tel:${n.number}`} style={{
                display: 'flex', alignItems: 'center', gap: '16px',
                padding: '16px', borderRadius: '16px', marginBottom: '12px',
                background: 'rgba(255,255,255,0.05)',
                border: `1px solid ${n.color}40`, textDecoration: 'none',
              }}>
                <div style={{
                  width: '48px', height: '48px', borderRadius: '50%',
                  background: `${n.color}20`, display: 'flex',
                  alignItems: 'center', justifyContent: 'center',
                  fontSize: '22px', border: `2px solid ${n.color}60`,
                }}>{n.icon}</div>
                <div>
                  <div style={{ color: '#fff', fontWeight: 700 }}>{n.label}</div>
                  <div style={{ color: n.color, fontWeight: 800, fontSize: '22px' }}>{n.number}</div>
                </div>
                <div style={{ marginLeft: 'auto', color: n.color, fontSize: '20px' }}>📲</div>
              </a>
            ))}
      ...
      {showForm && <IncidentForm onSubmit={sendMessage} onClose={() => setShowForm(false)} />}

      <div style={{
        width: '100%', maxWidth: '760px', height: '95vh',
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ color: '#fff', fontWeight: 800, fontSize: '17px', margin: 0 }}>ResQ Bot</h1>
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '11px', margin: 0 }}>AI Emergency First Responder</p>
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => setLang(l => l === 'hi' ? 'en' : 'hi')} style={{
              background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.2)',
              borderRadius: '10px', padding: '5px 10px', color: '#fff',
              fontSize: '11px', fontWeight: 700, cursor: 'pointer',
            }}>{lang === 'hi' ? '🇮🇳 HI' : '🇬🇧 EN'}</button>
            <button onClick={handleMute} style={{
              background: muted ? 'rgba(239,68,68,0.3)' : 'rgba(255,255,255,0.15)',
              border: '1px solid rgba(255,255,255,0.2)',
              borderRadius: '10px', padding: '5px 10px', color: '#fff',
              fontSize: '11px', fontWeight: 600, cursor: 'pointer',
            }}>{muted ? '🔇' : '🔊'}</button>
            <button onClick={() => setShowMap(m => !m)} style={{
              background: showMap ? 'rgba(59,130,246,0.4)' : 'rgba(255,255,255,0.15)',
              border: '1px solid rgba(255,255,255,0.2)',
              borderRadius: '10px', padding: '5px 10px', color: '#fff',
              fontSize: '11px', fontWeight: 600, cursor: 'pointer',
            }}>🗺️ Map</button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#4ade80', boxShadow: '0 0 8px #4ade80' }}></div>
        ...
        <div style={{
          ...
          {CRISIS_CARDS.map((c, i) => (
            <button key={i} onClick={() => sendMessage(c.prompt)} style={{
              background: 'rgba(255,255,255,0.05)',
              border: '1px solid rgba(255,255,255,0.1)',
              ...
              onMouseOver={e => e.currentTarget.style.background = 'rgba(230,57,70,0.25)'}
              onMouseOut={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
            >
              <span style={{ fontSize: '18px' }}>{c.icon}</span>
              <span style={{ fontSize: '10px', fontWeight: 600, color: 'rgba(255,255,255,0.7)' }}>{c.label}</span>
            </button>
          ))}
        </div>
            ...
            <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
              {msg.role === 'bot' && (
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '13px', marginRight: '8px', flexShrink: 0, marginTop: '4px',
                }}>🤖</div>
              )}
              <div style={{
                ...
                color: '#fff', fontSize: '13.5px', lineHeight: '1.7',
                boxShadow: msg.role === 'user' ? '0 4px 15px rgba(230,57,70,0.3)' : 'none',
              }}>
          ...
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{
                ...
                {[0, 1, 2].map(i => (
                  <div key={i} style={{
                    width: '7px', height: '7px', borderRadius: '50%', background: '#e63946',
        ...
        <div style={{
          ...
          <button onClick={handleSOS} disabled={sosActive} style={{
            ...
            marginBottom: '10px',
            boxShadow: sosActive ? '0 0 30px rgba(239,68,68,0.8)' : '0 4px 20px rgba(220,38,38,0.5)',
            animation: sosActive ? 'pulse 0.5s ease-in-out infinite' : 'none',
          }}>
            {sosActive ? '🆘 SOS SENT! Help is on the way...' : '🆘 SOS — EMERGENCY HELP'}
          </button>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <input
              ...
              placeholder={lang === 'hi' ? 'Emergency batao... (Hindi ya English)' : 'Describe your emergency...'}
              value={input}
